...
public class AttributeEventHandler {
    public static final float WATER_BOOST_MULTIPLIER = 2.5f;
    public static final float FLIGHT_SPEED_VERTICAL_MULTIPLIER = 0.666f;
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            if (player.worldObj.isRemote) {
                ...
                if (isFlying) {
                    float verticalFlightSpeed = ExtendedAPI.getAttribute(player, PlayerAttribute.FLIGHT_SPEED_VERTICAL) * FLIGHT_SPEED_VERTICAL_MULTIPLIER;
                    if (verticalFlightSpeed != 0) {
                        if (isMovingUp && !isMovingDown) {
                            player.motionY = verticalFlightSpeed;
            ...
            float horzMoveMultiplier = ExtendedAPI.getAttribute(player, EntityAttribute.JUMP_POWER_HORIZONTAL);
            ...
            float waterSwimBoost = ExtendedAPI.getAttribute(player, PlayerAttribute.SWIM_BOOST_WATER) * WATER_BOOST_MULTIPLIER;
    ...
    public void onRespawn(cpw.mods.fml.common.gameevent.PlayerEvent.PlayerRespawnEvent event) {
        ExtendedScriptEntityProperties.get(event.player).syncToClient();
        ExtendedScriptPlayerProperties.get(event.player).syncToClient();
    }
    ...
    public void onChangedDimension(cpw.mods.fml.common.gameevent.PlayerEvent.PlayerChangedDimensionEvent event) {
        ...
        PlayerAttributeTracker tracker = new PlayerAttributeTracker(event.player.getUniqueID());
        tracker.recalcAttributes(event.player);
        ExtendedScriptPlayerProperties.get(event.player).syncToClient();
//
...
//            ExtendedScripts.getPlayerProperties(player).syncToClient();
//        }
    }
    ...
    public void onJump(LivingEvent.LivingJumpEvent event) {
        if (event.entity instanceof EntityPlayer) {
            float jumpBoost = ExtendedAPI.getAttribute((EntityPlayer) event.entity, EntityAttribute.JUMP_POWER_VERTICAL);
            event.entity.motionY += jumpBoost * 0.2F;
        } else {
            ExtendedScriptEntityProperties properties = ExtendedScripts.getEntityProperties(event.entity);
    ...
    public void onLivingUpdate(LivingEvent.LivingUpdateEvent event) {
        ...
        if (!isFlying && !isSwimming) {
            ...
            if (event.entity instanceof EntityPlayer) {
                gravity = ExtendedAPI.getAttribute((EntityPlayer) event.entity, EntityAttribute.GRAVITY);
                downwardGravity = ExtendedAPI.getAttribute((EntityPlayer) event.entity, EntityAttribute.DOWNWARD_GRAVITY);
                upwardGravity = ExtendedAPI.getAttribute((EntityPlayer) event.entity, EntityAttribute.UPWARD_GRAVITY);
            ...
            if (event.entity instanceof EntityPlayer) {
                gravity = ExtendedAPI.getAttribute((EntityPlayer) event.entity, EntityAttribute.UNDERWATER_GRAVITY);
                downwardGravity = ExtendedAPI.getAttribute((EntityPlayer) event.entity, EntityAttribute.UNDERWATER_DOWNWARD_GRAVITY);
                upwardGravity = ExtendedAPI.getAttribute((EntityPlayer) event.entity, EntityAttribute.UNDERWATER_UPWARD_GRAVITY);
    ...
    private void applyEntityGravity(ExtendedScriptEntityProperties properties, Entity entity, float baseGravity, float gravity, float downwardGravity, float upwardGravity) {
        ...
        if (entity.motionY > 0) {
            entity.motionY += baseGravity - (baseGravity * (upwardGravity + 1));
        } else if (entity.motionY < 0) {
            entity.motionY += baseGravity - (baseGravity * (downwardGravity + 1));
        } else {
            if (upwardGravity < 0) {
                entity.motionY += baseGravity - (baseGravity * (upwardGravity + 1));
            }
...
class AttributeRecalcEventHandler implements AttributeRecalcEvent.Listener {
    ...
    public void onPost(EntityPlayer player, PlayerAttributeTracker playerAttributeTracker) {
        ...
        // Reset all PlayerAttributes to defaults before applying tracker values
        // This ensures removed modifiers (which won't be in tracker) reset to default
        for (PlayerAttribute attr : PlayerAttribute.values()) {
            playerProperties.set(attr, attr.getDefaultValue());
        }
        ...
        // Apply attributes from tracker (only contains non-zero values)
        for (ICustomAttribute attribute : playerAttributeTracker.getAttributes()) {
            String key = attribute.getAttribute().getKey();
            try {
                PlayerAttribute playerAttr = PlayerAttribute.valueOfFromSnakeCase(key);
                ICustomAttribute npcAttribute = playerAttributeTracker.getAttribute(key);
//                System.out.println("Set "+playerAttr.toString()+" to " + npcAttribute.getValue());
                AttributeDefinition definition = (AttributeDefinition) npcAttribute.getAttribute();
                if (definition.getValueType() == AttributeValueType.PERCENT) {
                    playerProperties.set(playerAttr, npcAttribute.getValue() / 100);
                } else {
                    playerProperties.set(playerAttr, npcAttribute.getValue());
                }

            } catch (IllegalArgumentException e) {
                try {
                    EntityAttribute entityAttr = EntityAttribute.valueOfFromSnakeCase(key);
                    ICustomAttribute npcAttribute = playerAttributeTracker.getAttribute(key);
//                    System.out.println("Set "+entityAttr.toString()+" to " + npcAttribute.getValue());
                    AttributeDefinition definition = (AttributeDefinition) npcAttribute.getAttribute();
                    if (definition.getValueType() == AttributeValueType.PERCENT) {
                        playerProperties.set(entityAttr, npcAttribute.getValue() / 100);
                    } else {
                        playerProperties.set(entityAttr, npcAttribute.getValue());
                    }
