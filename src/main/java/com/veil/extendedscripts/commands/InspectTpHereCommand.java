...
public class InspectTpHereCommand implements IVeilSubCommand {
    @Override
        ...
        if (args.length == 1 || args.length == 4 || args.length == 5) {
            ...
            if (args.length == 1) {
                if (inspectTag.getByte("TargetType") == 1) { // Block
                    ...
                    IPos pos = NpcAPI.Instance().getIPos(x, y, z);
                    IWorld world = (IWorld) NpcAPI.Instance().getIWorld(targetWorld);

                    IBlockData block = world.getBlockData(pos);
                    ...
                    int playerX = (int) playerSender.posX;
                    int playerY = (int) playerSender.posY;
                    int playerZ = (int) playerSender.posZ;

                    IPos dest = NpcAPI.Instance().getIPos(playerX, playerY, playerZ);

                    world.setBlock(dest, block);

                    targetWorld.removeTileEntity(x, y, z);
                    targetWorld.setBlockToAir(x, y, z);

                    sender.addChatMessage(ChatUtils.fillChatWithColor("§aTeleported to target to you!"));
                    ...
                    foundEntity.setPosition(playerSender.posX, playerSender.posY, playerSender.posZ);
                    sender.addChatMessage(ChatUtils.fillChatWithColor("§aTeleported to target to you!"));
                }
            }
            if (args.length == 4) {
                if (inspectTag.getByte("TargetType") == 1) { // Block
                    ...
                    double[] destArray = InspectCommand.getCoordinatesFromArgument(
                        new String[] {args[1], args[2], args[3]},
                        new double[] {playerX, playerY, playerZ},
                        sender
                    );

                    IPos dest = NpcAPI.Instance().getIPos(
                        destArray[0],
                        destArray[1],
                        destArray[2]
                    );
                    ...
                    sender.addChatMessage(ChatUtils.fillChatWithColor("§aTeleported the target relative to target!"));
                } else if (inspectTag.getByte("TargetType") == 2) { // Entity
                    Entity foundEntity = InspectCommand.getEntityFromInspectTag(inspectTag, targetWorld, sender);
                    ...
                    double[] dest = InspectCommand.getCoordinatesFromArgument(
                        new String[] {args[1], args[2], args[3]},
                        new double[] {playerSender.posX, playerSender.posY, playerSender.posZ},
                        sender
                    );

                    foundEntity.setPosition(dest[0], dest[1], dest[2]);
                    sender.addChatMessage(ChatUtils.fillChatWithColor("§aTeleported the target relative to target!"));
                }
                ...
                if (inspectTag.getByte("TargetType") == 1) { // Block
                    ...
                    IWorld destWorld = (IWorld) NpcAPI.Instance().getIWorld((int) destArray[3]);
                    destWorld.setBlock(dest, block);
                    ...
                    foundEntity.setPosition(dest[0], dest[1], dest[2]);
                    foundEntity.travelToDimension((int)dest[3]);
                    sender.addChatMessage(ChatUtils.fillChatWithColor("§aTeleported the target relative to target!"));
                }
            }
        } else {
